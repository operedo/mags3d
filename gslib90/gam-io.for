...
c read two lines at once
      real,allocatable  ::    var2lines(:,:)

      real*8    avg(MV),ssq(MV)
      integer   ivar(MV),num(MV),ivc(500),indflag(500)
      integer   buffer,bufferl
...
c
      ...
      buffer=nx
      bufferl=buffer-1
      allocate(var2lines(nvari,buffer),stat=test)
      if (test.ne.0) then
            write(*,*) 'Error: Allocation failed due to ',
     +                 'insufficient memory!', test
            stop
...
c            do iy=1,ny
c            do ix=1,nx
c                  if(is.ne.isim) then
c                        read(lin,'()',err=99)
c                  else
c                        read(lin,*,err=99) (var(i),i=1,nvari)
c                        do iv=1,nvar
c                              i=ivar(iv)
c                              index=ix+(iy-1)*nx+(iz-1)*nxy+(iv-1)*nxyz
c                              vr(index) = var(i)
c                              if(var(i).ge.tmin.and.var(i).lt.tmax) then
c                                    num(iv) = num(iv) + 1
c                                    avg(iv) = avg(iv) + dble(var(i))
c                                    ssq(iv) = ssq(iv)
c     +                                      + dble(var(i)*var(i))
c                              end if
c                        end do
...
c            do ix=1,nx-15,16
c            do ix=1,nx-31,32
            do ix=1,nx-bufferl,buffer
                  if(is.ne.isim) then
                        read(lin,'()',err=99)
                  else
                        read(lin,*,err=99) var2lines(:,:)

                        do irow=0,bufferl

                        do iv=1,nvar
                              i=ivar(iv)
                              index=ix+irow+(iy-1)*nx+(iz-1)*nxy+(iv-1)
     + *nxyz
                              vr(index) = var2lines(i,irow+1)
c                              if(var2lines(i,irow+1).ge.tmin.and.
c     + var2lines(i,irow+1).lt.tmax) then
                                    num(iv) = num(iv) + 1
                                    avg(iv) = avg(iv) + dble(var2lines(i
     + ,irow+1))
                                    ssq(iv) = ssq(iv)
     +                                      + dble(var2lines(i,irow+1)
     + *var2lines(i,irow+1))
c                              end if
            ...
            do ix2=ix,nx
                  if(is.ne.isim) then
                        ...
                        do iv=1,nvar
                              i=ivar(iv)
                              index=ix2+(iy-1)*nx+(iz-1)*nxy+(iv-1)*nxyz
                              vr(index) = var(i)
     ...
     +                                      + dble(var(i)*var(i))
      ...
      deallocate(var2lines)

      call cpu_time(timeEnd)
      write(*,*)'Time read file total=',(timeEnd-timeIni)
