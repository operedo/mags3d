...
//#endif
	...
	int retTarget = genTargetVariogram(nx,ny,nz,xlo,ylo,zlo,h,a,imageZero);
	...
	TYPE cost = costFunctionGSLIB(nlags,expVariogram,npairs,0);
	...
	for(i=1;i<=iters;i++){
		if((currentCost/initialCost)<=tol){
			...
			modifiedRadius = averageWeights(	
				...
				currentCost = costFunctionGSLIB(nlags,expVariogram,npairs,i);
			}
			...
			ret = genMovingAverageImage(imageZero,weightPerturbed,nx,ny,nz,nxExt,nyExt,nzExt,neighRadius,neighSide,params,i,report);
			tempCost = costFunctionGSLIB(nlags,expVariogram,npairs,i);
			...
			if(probability>ran){
				...
				//if(i%50==0)
				fprintf(stdout,"accepted(r=%f,new=%f,old=%f)\ttempCost=%f\tcurrCost=%f\t%f\n",modifiedRadius,newval,oldval,tempCost,currentCost,currentCost/initialCost);
				ret = modifyWeights(
							weight,
							neighRadius,
							neighSide,
				...
				//if(i%50==0)
				fprintf(stdout,"rejected(r=%f,new=%f,old=%f)\ttempCost=%f\tcurrCost=%f\t%f\n",modifiedRadius,newval,oldval,tempCost,currentCost,currentCost/initialCost);
				ret = modifyWeights(
							weightPerturbed,
							neighRadius,
							neighSide,
